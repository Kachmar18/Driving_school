...
public class InstructorPracticeSchedule implements Localisation {
    public Label schedulePracticeLabel;
    ...
    public TableColumn<PracticeData, String> dayPracticeColumn, hourPracticeColumn, routePracticeColumn, durationPracticeColumn, statusPracticeColumn, studentPracticeColumn, phonePracticeColumn, operationsPracticeColumn;
    ...
    public void loadData() {
        try {
            ...
            String sql = """
                    ...
                    Практичні_заняття.Дата,\s
                    Практичні_заняття.Час,\s
                    Практичні_заняття.Маршрут,\s
                    Практичні_заняття.`Тривалість, год`,\s
                    Практичні_заняття.Статус,\s
                    Студент.`Ім'я`,\s
                    Студент.Прізвище,\s
                    Студент.Номер_телефону\s
                    FROM Практичні_заняття\s
                    JOIN Студент ON Практичні_заняття.id_студент = Студент.id_студент\s
                    JOIN Інструктор ON Практичні_заняття.id_інструктор = Інструктор.Id_інструктор\s
                    WHERE Інструктор.Id_інструктор = ?;\s""";
            ...
            while (resultSet.next()) {
                PracticeData practice = new PracticeData( // Create CourseData objects and add them to the ObservableList
            ...
            ObservableList<PracticeData> filteredData = data.filtered(practice ->
                    practice.getDate().toLowerCase().contains(keyword) ||
                            ...
                            practice.getPhone().toLowerCase().contains(keyword)
            );
    ...
    public static class PracticeData {
        ...
        private final SimpleStringProperty date, time, route, duration, status, student, phone;
        public PracticeData(String id_practice, String date, String time, String route, String duration, String status, String student, String phone) {
    ...
    public Callback<TableColumn<PracticeData, String>, TableCell<PracticeData, String>> createOperationsCellFactory() {
        return new Callback<>() {
            @Override
            public TableCell<PracticeData, String> call(TableColumn<PracticeData, String> param) {
                return new TableCell<>() {
                    ...
                    protected void updateItem(String item, boolean empty) {
                        ...
                        if (empty) {
                            ...
                            iconsContainer.getChildren().addAll(editIcon, doneIcon);
    ...
    private void handleEditAction(int rowIndex) throws IOException {
        ...
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/autoschool/autoschool_javafx/Instructor/instructor-new-practice.fxml"));
        Parent fxml = loader.load();

        InstructorNewPractice instructorNewPractice = loader.getController();
        instructorNewPractice.setCurrentLanguage();
        instructorNewPractice.setInstructorMenuWindow(instructorMenuWindow);

        instructorNewPractice.statusLabel.setVisible(true);
        instructorNewPractice.statusComboBox.setVisible(true);
        instructorNewPractice.createNewPracticeTitleText.setText(LanguageManager.getString("updatePracticeTitleText"));

        PracticeData selectedPractice = schedulePracticeTableView.getItems().get(rowIndex);
        instructorNewPractice.setEditPracticeData(selectedPractice, selectedPractice.getId_practice());
