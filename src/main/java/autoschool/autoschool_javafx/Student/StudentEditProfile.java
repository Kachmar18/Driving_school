...
public class StudentEditProfile implements Localisation {
    ...
    public void setStudentMenuWindow(StudentMenuWindow studentMenuWindow) {
        this.studentMenuWindow = studentMenuWindow;
    }
    ...
    public void selectInformationFromDatabase() throws SQLException {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(DataBase.sql_selectInformationStudentAccount)) {
    ...
    public void editAccount() throws SQLException {
        if (areAllFieldsFilled()) {
            ...
            if (userConfirmed) {
                ...
                String errorMessage = updateStudentProfileInDatabase();
    ...
    public String updateStudentProfileInDatabase() throws SQLException {
        Connection connection = DriverManager.getConnection(DataBase.DATABASEURL, DataBase.USERNAME, DataBase.PASSWORD);
        try {
            ...
            try (PreparedStatement statement = connection.prepareStatement(DataBase.sql_editProfileStudentAccount)) {
                ...
                statement.setString(10, String.valueOf(StudentMenuWindow.id_student));

                int rowsAffected = statement.executeUpdate(); // Execute the update
                if (rowsAffected > 0) { // Check the result of the update
                    studentMenuWindow.updateStudentInfo(firstNameTextField.getText(), lastNameTextField.getText());
                    return null; // Success
                } else {
    ...
    private boolean isDuplicateEntry(Connection connection) throws SQLException {
        String selectQuery;
        if (!phoneTextField.getText().equals(originalPhone) && emailTextField.getText().equals(originalEmail)) {
            selectQuery = "SELECT * FROM `студент` WHERE `Номер_телефону` = ?";
        } else if (phoneTextField.getText().equals(originalPhone) && !emailTextField.getText().equals(originalEmail)) {
            selectQuery = "SELECT * FROM `студент` WHERE `E-mail` = ?";
        } else {
            selectQuery = "SELECT * FROM `студент` WHERE `Номер_телефону` = ? OR `E-mail` = ?";
        }
    ...
    public void returnButton() throws IOException {
        ...
        settingsWindow.setStudentMenuWindow(studentMenuWindow);
